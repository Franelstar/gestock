...
class MY_Model extends CI_Model{
    ...
    public function counte($condition=null) {
		if($condition != null){
	...
	public function get_record_sup() {
		// aucun parametre passe a la fontion 
		
        $query = $this->db->query("SELECT * FROM achat WHERE qnt_rest > 0 AND achat_status = 1");
		return $query->result_array();
    }
	...
	*/
	...
	public function get_joind2($join1, $join2, $row1, $row2, $row3, $row4, $condition = NULL, $as = ""){
		if(empty($condition)){
			$this->db->select('*'.$as);
			$this->db->from($this->_table);
			$this->db->join($join1, $row1.'='.$row2);
			$this->db->join($join2, $row3.'='.$row4);
			$query = $this->db->get();
			return $query->result_array();
		}else{
			$this->db->select('*'.$as);
	...
	public function get_joind3($join1, $join2, $join3, $row1, $row2, $row3, $row4, $row5, $row6, $condition = NULL){
		if(empty($condition)){
			$this->db->select('*');
			$this->db->from($this->_table);
			$this->db->join($join, $row1.'='.$row2);
			$this->db->join($join2, $row3.'='.$row4);
			$this->db->join($join3, $row5.'='.$row6);
			$query = $this->db->get();
			return $query->result_array();
		}else{
			$this->db->select('*');
	...
	public function get_joind4($join1, $join2, $join3, $join4, $row1, $row2, $row3, $row4, $row5, $row6, $row7, $row8, $condition = NULL, $as = ""){
		if(empty($condition)){
			$this->db->select('*'.$as);
			$this->db->from($this->_table);
			$this->db->join($join, $row1.'='.$row2);
