			...
			<div class="review-content-section row">
				<form method="POST" action="" class="row">
					<div class="chosen-select-single mg-b-20 col-xs-8">
						<select id="annee" class="chosen-select" name="annee">
							<?php for($i = 0; $i <= 12; $i++){
								$b = explode('-',$date);
								if($b[2] == 0){
									$b[2] = 1;
								...
								$comp = date('Y', strtotime('+'.-$i.' year', $time));
								if($comp >= $petite_date){ ?>
									<option <?php if(isset($active_mois) and  $active_mois== $i){echo 'selected';} ?> value="<?php echo $i; ?>"><?php echo af_date_date2(date('Y-m-d'), -$i); ?></option>
								<?php } ?>
							<?php } ?>
						</select>
					</div>
